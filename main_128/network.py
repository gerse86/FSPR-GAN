...
class MappingNetwork(nn.Module):
    ...
    def forward(self, z: torch.Tensor):
        z = F.normalize(z, dim=1)  # Normalize latent code
        return self.net(z)


class Generator(nn.Module):
    """ 3D generator: upsamples from 4x4x4 constant volume to 128x128x128. """
    def __init__(self, log_resolution: int, d_latent: int, n_features: int = 4, max_features: int = 128):
        super().__init__()
        # log_resolution-2 = 7-2 = 5, i in range(5,4,3,2,1,0)
        # Compute feature maps: [128, 64, 32, 16, 8, 4]
        features = [min(max_features, n_features * (2 ** i)) for i in range(log_resolution - 2, -1, -1)]
        self.n_blocks = len(features)

        # Initial constant volume (1, 128, 4, 4, 4)
        self.initial_constant = nn.Parameter(torch.randn((1, features[0], 4, 4, 4)))

        self.style_block = StyleBlock(d_latent, features[0], features[0])
        self.to_volume = ToVolume(d_latent, features[0])

        # Progressive generator blocks
    ...
    def forward(self, w: torch.Tensor, input_noise: List[Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]]):
        ...
        for i in range(1, self.n_blocks):
            ...
            volume = self.up_sample(volume) + volume_new  # Accumulate volumes

        return self.activation(volume)


class GeneratorBlock(nn.Module):
    """ Generator block with two style blocks and a volume output. """
...
class StyleBlock(nn.Module):
    """ Style block with weight-modulated convolution and noise injection. """
    def __init__(self, d_latent: int, in_features: int, out_features: int):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, w: torch.Tensor, noise: Optional[torch.Tensor]):
        ...
        if noise is not None:
            x = x + self.scale_noise[None, :, None, None, None] * noise  # Inject noise
        return self.activation(x + self.bias[None, :, None, None, None])


class ToVolume(nn.Module):
    """ Converts feature maps to a single-channel 3D volume. """
...
class Conv3dWeightModulate(nn.Module):
    ...
    def forward(self, x: torch.Tensor, s: torch.Tensor):
        b, _, d, h, w = x.shape
        s = s[:, None, :, None, None, None]          # Expand style for element-wise multiplication
        weights = self.weight()[None, :, :, :, :, :]
        weights = weights * s                        # Modulate weights

        if self.demodulate:
            sigma_inv = torch.rsqrt((weights ** 2).sum(dim=(2, 3, 4, 5), keepdim=True) + self.eps)
            weights = weights * sigma_inv            # Demodulate
...
class Discriminator(nn.Module):
    """ 3D discriminator for single-channel volumes with multi-scale std features. """
    def __init__(self, log_resolution: int, n_features: int = 4, max_features: int = 128):
        super().__init__()
        # Convert input volume to feature maps
        ...
        final_features = features[-1] + 33          # Add 33 extra channels from std processing

        self.conv = EqualizedConv3d(final_features, final_features, 3, padding=1)
        self.final = EqualizedLinear(4 * 4 * 4 * final_features, 1)  # Flatten to scalar

        # Additional std feature extraction network
    ...
    def forward(self, x: torch.Tensor):
        ...
        _, std = self.std_dev(x)                     # Extract std feature map
        std = self.std_conv_layers(std)              # Process std to higher dimensions

        x = self.blocks(x)
        x, _ = self.std_dev(x)                       # Apply std again on feature maps

        x = torch.cat([x, std], dim=1)               # Concatenate std features
        x = self.conv(x)
...
class DiscriminatorBlock(nn.Module):
    """ 3D discriminator block with residual connection. """
...
class MiniBatchStdDev3D(nn.Module):
    """
    Computes minibatch standard deviation across spatial dimensions.
    Returns the concatenated feature map and the std map separately.
    """
    ...
    def forward(self, x: torch.Tensor):
        ...
        return torch.cat([x, std], dim=1), std            # Concatenate and return std separately
...
class Smooth3D(nn.Module):
    """ 3D smoothing using a fixed Gaussian blur kernel. """
    def __init__(self):
        super().__init__()
