...
class PorosityDataset(torch.utils.data.Dataset):
    ...
    Supports both uint8 (binary) and float32 (pre-normalized) data formats.
    """
    ...
    def __getitem__(self, index: int) -> torch.Tensor:
        ...
        # Determine data type based on file location
...
class DiscriminatorLoss(nn.Module):
    """ Discriminator loss using the non-saturating logistic loss. """
    def __init__(self):
        super().__init__()
...
class GeneratorLoss(nn.Module):
    """ Generator loss using the non-saturating logistic loss. """
    def __init__(self):
        super().__init__()
...
class TransitionDistanceLoss(nn.Module):
    """ Penalizes intermediate values away from -1 or +1, encouraging binary-like outputs. """
    def __init__(self):
        super().__init__()
