"""
3D pore network metrics extraction and analysis utilities.
Supports porosity, throat statistics, two-point correlation (TPC), and pore/throat size distributions.
"""
...
def process_file(file_path, shape=(128, 128, 128), dtype=np.uint8):
    """
    Process a single 3D .raw file and extract pore network properties.

    Returns:
        dict: Contains pore volumes, surface areas, equivalent diameters,
              throat lengths, surface areas, equivalent diameters, and the volume arrays.
    """
    vol_3d = np.fromfile(file_path, dtype=dtype).reshape(shape)
    vol_3d_inverted = vol_3d == 0  # Pores are True, solid is False
...
def calculate_porosity_and_stats(pn, shape=(128, 128, 128), voxel_size=1):
    """
    Calculate porosity and pore/throat statistics from a pore network.

    Returns:
        tuple: (total_porosity, connected_porosity, total_pores, connected_pores, num_throats)
    """
...
def process_stats_file(args):
    """
    Helper for parallel processing: compute statistics for a single file.

    Returns:
        (file_name, stats_tuple) or (None, None) on failure.
    """
...
def calculate_stats_for_folder(folder_path, shape=(128, 128, 128)):
    """
    Process all .raw files in a folder and compute average porosity/throat statistics.

    Returns:
        tuple: (file_names, list_of_stats, average_stats_tuple)
    """
...
def process_tpc_file(args):
    """
    Helper for parallel processing: compute two‑point correlation for a single file.

    Returns:
        (distance_array, pdf_array) or (None, None) on failure.
    """
...
def get_all_tpc_data(folder_path, shape=(128, 128, 128), bins=120, voxel_size=1):
    """
    Collect two‑point correlation data from all .raw files in a folder.

    Returns:
        (list_of_distances, list_of_pdfs)
    """
    file_paths = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.raw')]
    all_distances = []
    all_pdfs = []
...
def calculate_average_tpc(all_distances, all_pdfs):
    """
    Interpolate all TPC curves onto a common distance grid and compute the average PDF.

    Returns:
        (common_distance, average_pdf) or (None, None) if no data.
    """
...
def align_tpc_data(real_dist, real_pdf, fake_dist, fake_pdf):
    """
    Align real and fake TPC curves onto a common distance grid.

    Returns:
        (common_distance, aligned_real_pdf, aligned_fake_pdf) or (None, None, None) on failure.
    """
...
def calculate_tpc_relative_error(real_pdf, fake_pdf, epsilon=1e-8):
    """
    Compute the relative error between real and fake TPC curves.

    Returns:
        relative_error array (percentage) or None if lengths mismatch.
    """
...
def extract_pore_throat_data(file_path, shape):
    """
    Extract pore and throat equivalent diameters from a single file.

    Returns:
        (pore_diameters, throat_diameters) or (None, None) on failure.
    """
    try:
...
def extract_data_parallel(folder_path, shape):
    """
    Collect pore and throat diameter data from all .raw files in a folder using parallel processing.

    Returns:
        (list_of_pore_diameters, list_of_throat_diameters)
    """
    file_paths = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.raw')]
    pore_data_list = []
    throat_data_list = []
