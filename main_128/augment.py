"""
3D augmentation pipeline for volumetric data.
Includes geometric transformations, morphological transformations, and numerical perturbation.
"""
...
def matrix(*rows, device=None):
    """ Constructs a batch of transformation matrices from rows. """
...
def get_pore_boundary_mask_3d(volumes: torch.Tensor, kernel_size=3) -> torch.Tensor:
    """
    Computes a binary mask indicating pore boundary voxels (pore voxels adjacent to solid).
    Used for numerical perturbation to apply different noise levels at boundaries.
    Uses a cross-shaped 3D kernel (6-neighborhood).
    """
    ...
    kernel[0, 0, center, center, :] = 1.0  # along W
    kernel[0, 0, center, :, center] = 1.0  # along H
    kernel[0, 0, :, center, center] = 1.0  # along D
...
def gaussian_perturb_3d(
    ...
    """
    Applies Gaussian noise as a numerical perturbation with different standard deviations
    for boundary and interior voxels. Clamps values to keep them within the valid range.
    """
    device = volumes.device
...
def numeric_perturb_3d(
    ...
    """
    Convenience function: computes boundary mask and applies numerical perturbation
    (Gaussian noise) to the input volumes.
    """
...
class AugmentPipe3D(nn.Module):
    """
    Adaptive augmentation pipeline for 3D volumes.
    Supports geometric (flips, 90° rotations), morphological (erosion/dilation),
    and numerical (Gaussian noise) perturbations with adaptive control.
    """
    def __init__(self,
                 # 1. Geometric transformations
                 xflip=0.6,
    ...
    def update_ada_stats(self, real_pred):
        """ Updates the adaptive statistics using discriminator predictions on real data. """
    ...
    def adjust_p(self, target=0.6, rate=0.001):
        """
        Adjusts the augmentation probability p based on the adaptive statistics.
        Returns the current signal and the new p value.
        """
    ...
    def forward(self, volumes):
        """ Applies augmentations to the input volumes. """
        assert isinstance(volumes, torch.Tensor) and volumes.ndim == 5, "Input must be a 5D tensor [B, C, D, H, W]"
        ...
        # X-axis flip: probability is (xflip * p)
        if self.xflip > 0:
        ...
        # Y-axis flip: probability is (yflip * p)
        if self.yflip > 0:
        ...
        # Z-axis flip: probability is (zflip * p)
        if self.zflip > 0:
        ...
        # 90-degree rotation: randomly select one of the three axes
        if self.rotate90 > 0:
